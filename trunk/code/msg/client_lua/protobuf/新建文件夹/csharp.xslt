...
// Note: requires additional types generated from: <xsl:value-of select="."/></xsl:template>
  ...
  -->public partial class <xsl:call-template name="pascal"/> : msg_base<!--
  ...
  --><xsl:if test="$optionPreObservable">, global::System.ComponentModel.INotifyPropertyChanging</xsl:if>
  {
	...
	<xsl:if test="not(starts-with(name, 'D'))">
	public override int _msgid(){return MsgDef.msgid_<xsl:value-of select="name"/>;}</xsl:if>
  }
  </xsl:template>
